    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <a href="{{route('admin.property_facilities.index')}}">{{__('Property Facilities')}}</a>
                </li>
                <li class="breadcrumb-item active">{{__('Edit Facility')}}</li>
                <!-- Basic table -->
            ...
            <div class="col-xl">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">{{__('Edit Facility')}}</h5>
                    </div>
                    <div class="card-body">
                                            ...
                                            <div id="accordion{{ $locale }}" class="accordion-collapse collapse @if ($index === 0) show @endif" data-bs-parent="#accordionExample">
                                                <div class="accordion-body">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" name="name[{{ $locale }}]" value="{{ $facility->getTranslation('name', $locale) }}" id="name_{{ $locale }}" placeholder="{{ __('Name in ') . strtoupper($locale) }}" required>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="col-md-6 mb-6">
                                <div class="form-group mb-3">
                                    ...
                                    <select name="status" id="status" class="form-select">
                                        <option value="0" {{$facility->status==0?'selected':''}}>{{__('Pending')}}</option>
                                        <option value="1" {{$facility->status==1?'selected':''}}>{{__('Published')}}</option>
                                        <option value="2" {{$facility->status==2?'selected':''}}>{{__('draft')}}</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6 mb-6">
                                ...
                                <select class="selectpicker w-100 show-tick" id="selectpickerIcons" name="icon" data-icon-base="fa-solid"   data-style="btn-default">
                                    @foreach($icons as $icon)
                                        <option data-icon=" {{$icon->icon}} " class="mb-2" value="{{$icon->icon}}" {{$facility->icon==$icon->icon?'selected':''}}>{{$icon->icon}}</option>
                                    @endforeach
...
@section('scripts')
    ...
    <script>
        ...
        var data_url='{{ route('admin.property_facilities.update',$facility->id)}}'

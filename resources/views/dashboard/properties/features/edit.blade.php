    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{__('Edit Feature')}}</li>
                <!-- Basic table -->
                                            ...
                                            <div id="accordion{{ $locale }}" class="accordion-collapse collapse @if ($index === 0) show @endif" data-bs-parent="#accordionExample">
                                                <div class="accordion-body">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" name="name[{{ $locale }}]" value="{{ $feature->getTranslation('name', $locale) }}" id="name_{{ $locale }}" placeholder="{{ __('Name in ') . strtoupper($locale) }}" required>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div class="col-md-6 mb-6">
                                ...
                                <select class="selectpicker w-100 show-tick" id="selectpickerIcons" name="category_id" data-icon-base="fa-solid"   data-style="btn-default">
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}" class="mb-2" {{$feature->category_id==$category->id?'selected':''}}>{{$category->getTranslation('name', $locale) }}</option>
                                    @endforeach
                            ...
                            <div class="col-md-6 mb-6">
                                ...
                                <select class="selectpicker w-100 show-tick" id="selectpickerIcons" name="icon" data-icon-base="fa-solid"   data-style="btn-default">
                                    @foreach($icons as $icon)
                                        <option data-icon=" {{$icon->icon}} " class="mb-2" value="{{$icon->icon}}" {{$feature->icon==$icon->icon?'selected':''}}>{{$icon->icon}}</option>
                                    @endforeach
...
@section('scripts')
    ...
    <script>
        ...
        var data_url='{{ route('admin.property_features.update',$feature->id)}}'

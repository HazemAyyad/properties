    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <a href="{{route('admin.plans.index')}}">{{__('Plans')}}</a>
                </li>
                <li class="breadcrumb-item active">{{__('Edit Plan')}}</li>
                <!-- Basic table -->
                    ...
                    <div class="card-body">
                        <form id="mainAdd" method="post" action="javascript:void(0)" >
                                            ...
                                            <div id="accordion{{ $locale }}" class="accordion-collapse collapse @if ($index === 0) show @endif" data-bs-parent="#accordionExample">
                                                <div class="accordion-body">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" name="title[{{ $locale }}]"  value="{{ $plan->getTranslation('title', $locale) }}" id="title_{{ $locale }}" placeholder="{{ __('Name in ') . strtoupper($locale) }}" required>
                                                    </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" class="form-control" name="slug" id="slug" value="{{ $plan->slug }}" placeholder="e.g. trial">
                            </div>
                            <div class="form-group mb-3">
                                <label class="form-label" for="duration_months">{{__('Duration (months)')}}</label>
                                <input type="number" class="form-control" name="duration_months" id="duration_months" value="{{ $plan->duration_months }}" placeholder="{{__('Leave empty if N/A')}}" min="0" step="1">
                            </div>
                            <div class="form-group mb-3">
                                <label class="form-label" for="number_of_properties">{{__('Number of properties')}}</label>
                                <input type="number" class="form-control" name="number_of_properties" id="number_of_properties" value="{{ $plan->number_of_properties }}" placeholder="-1 = {{__('Unlimited')}}">
                            </div>
                            <div class="form-group mb-3">
                                <label class="form-label" for="price_monthly">{{__('price monthly')}}</label>
                                <input type="text" class="form-control" name="price_monthly" value="{{$plan->price_monthly}}" id="price_monthly" placeholder="{{__('price monthly')}}" >
                            </div>
                            <div class="form-group mb-3">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <select id="status" required name="status" class="form-control select2">
                                        <option value="">{{ __('Select Status') }}</option>

                                        <option value="0" {{$plan->status==0?'selected':''}}>{{__('Not Active')}}</option>
...
@section('scripts')
    ...
    <script>
        ...
        var data_url='{{ route('admin.plans.update',$plan->id)}}'

...
<div class="row">
    <div class="col-md-4">
        <div class="form-group mb-3">
            ...
            <select name="building_age" class="form-control form-select">
                ...
                <option value="new" {{ old('building_age', optional($info)->building_age) == 'new' ? 'selected' : '' }}>{{ __('New') }}</option>
    ...
    <div class="col-md-4">
        <div class="form-group mb-3">
            ...
            <select name="rooms" class="form-control form-select">
                <option value="">{{ __('Select') }}</option>
                @for($i = 1; $i <= 5; $i++) <option value="{{ $i }}" {{ old('rooms', optional($info)->rooms) == $i ? 'selected' : '' }}>{{ $i }}</option> @endfor
                <option value="5+" {{ old('rooms', optional($info)->rooms) == '5+' ? 'selected' : '' }}>5+</option>
            </select>
        </div>
    </div>
    <div class="col-md-4">
    ...
    <div class="col-md-4">
        <div class="form-group mb-3">
            <label class="form-label">{{ __('Office Area (m²)') }} ({{ __('from') }})</label>
            <input type="number" step="0.01" name="size" class="form-control" value="{{ old('size', optional($info)->size ?? '') }}">
        </div>
    </div>
    <div class="col-md-4">
        <div class="form-group mb-3">
            <label class="form-label">{{ __('Office Area (m²)') }} ({{ __('to') }})</label>

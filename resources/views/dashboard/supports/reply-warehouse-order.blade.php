...
@section('content')
                ...
                <div class="col-md-5 mt-5">
                    <div class="card warehouse_package">
                        <div class="card-header">
                            ...
                            <a class="fw-bold" data-magnify="gallery" data-caption="box open"   href="{{$package->photos[0]['photo']}}" id="link_open_1">
                                more Photo
                            </a>
                            @foreach($package->photos as $iem)
                                @if($loop->first)
                                    <a data-magnify="gallery" data-caption="box open" data-group="open_{{$package->id}}" href="{{$iem->photo}}">
                                        <img src="{{$iem->photo}}">
                                    </a>
                                @else
                                    <a data-magnify="gallery" data-caption="box open" data-group="open_{{$package->id}}" href="{{$iem->photo}}" style="display: none">
                                        <img src="{{$iem->photo}}">
                                    </a>
                                @endif
                            @endforeach
                        ...
                        <div class="card-body">
                            ...
                            @if($package->status==0)
                                <div class="content-package text-center">
                                    <p class="package_no"><span class="fw-bold">Order No :</span>#{{$package->order_no}}</p>

                                    <p class="arrived_date"><span class="fw-bold">Request Date :</span> ({{$package->created_at->format('m/d/Y')}} )</p>


                                    <a href="javascript:;" onclick="viewDetails(this,{{$package->id}})" class="btn btn-warning"  >
                                        View
                                    </a>
                            ...
                            @elseif($package->status==1)
                                <div class="content-package text-center">
                                    <p class="package_no"><span class="fw-bold">Order No :</span>#{{$package->order_no}}</p>
                                    <p class="track_no"><span class="fw-bold">weight (L x W x H) QTY:</span><br>
                                        @foreach($package->package_items as $item)

                                            <span class="d-block">{{$item['weight']}} {{$item['unit_weight']=='1'?'lbs':'KG'}} ({{$item['length']}}  x {{$item['width']}}  x {{$item['height']}} ) {{$item['unit_dimension']=='1'?'in':'cm'}} {{$item->qty}}</span>

                                        @endforeach
                                    </p>
                                    <p class="goods_value"><span class="fw-bold">Goods Value :</span>${{$package->total_price}}</p>
                                    <p class="goods_value"><span class="fw-bold">Package :</span>{{$package->count_package}}</p>
                            ...
                            @elseif($package->status==2)
                                <div class="content-package text-center">
                                    ...
                                    {{--                                        <a href="{{route('address-to-warehouse',$package->id)}}" class="btn btn-main"  >--}}
                                    {{--                                           Ship now--}}
                                    {{--                                        </a>--}}
                                    <a href="javascript:;" onclick="viewDetails(this,{{$package->id}})" class="btn btn-warning"  >
                                        View
    ...
    <!-- / Content -->
    <div class="modal fade" id="details_modal"    tabindex="-1" role="dialog" aria-labelledby="basicModal"
         aria-hidden="true">
                ...
                <div class="modal-header text-center" style="border-bottom: 0px;">
                    <div class="row" style="width: 100%;">
                        <div class="col-12 text-center">
                            <h4 class="modal-title text-center fw-bold" id="myModalLabel">{{__('Details')}}</h4>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-12" id="div_details">
                            <table class="table" id="table_packages">
                                <thead>
                                <tr>
                                    <td class="fw-bold">TR No</td>
...
@section('scripts')
    ...
    <script type="text/javascript">
        function viewDetails (element , id){


            $.ajax({
                url:'{{ route('warehouse_orders.get-details-order','') }}'+'/'+id,
                type:'GET',
                success:function (data) {
                    $('#details_modal').modal('show');
                    $('table').find('tbody').empty();
                    $.each(data.package.original_packages, function (key, val) {

                        $('table').find('tbody').append([
                            '<tr>',
                            '<td> '+val.track_no+'</td>',
                            '<td>'+val.name_item+'</td>',
                            '<td>'+val.price_custom_value+'</td>',

    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{__('Edit Say')}}</li>
                <!-- Basic table -->
                                            ...
                                            <div id="accordion{{ $locale }}" class="accordion-collapse collapse @if ($index === 0) show @endif" data-bs-parent="#accordionExample">
                                                <div class="accordion-body">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" name="name[{{ $locale }}]"  value="{{ $say->getTranslation('name', $locale) }}" id="name_{{ $locale }}" placeholder="{{ __('Name in ') . strtoupper($locale) }}" required>
                                                    </div>

                                                    <div class="form-group mb-3">
                                                        <label class="form-label" for="position_{{ $locale }}">{{__('Position')}} ({{ strtoupper($locale) }})</label>
                                                        <input type="text" class="form-control" name="position[{{ $locale }}]" value="{{ $say->getTranslation('position', $locale) }}" id="position_{{ $locale }}" placeholder="{{ __('Position') . strtoupper($locale) }}" required>
                                                    </div>
                                                    <div class="form-group mb-3">
                                                        <label class="form-label" for="description_{{ $locale }}">{{__('description')}}  ({{ strtoupper($locale) }})</label>
                                                        <textarea name="description[{{ $locale }}]" id="description_{{ $locale }}"  class="form-control" rows="3">{{ $say->getTranslation('description', $locale) }}</textarea>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        var data_url='{{ route('admin.people_say.update',$say->id)}}'

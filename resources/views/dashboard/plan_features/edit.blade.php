    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <a href="{{route('admin.plan_features.index')}}">{{__('Plan Features')}}</a>
                </li>
                <li class="breadcrumb-item active">{{__('Edit Plan Feature')}}</li>
                <!-- Basic table -->
                            ...
                            <div class="col-md-12">
                                            ...
                                            <div id="accordion{{ $locale }}" class="accordion-collapse collapse @if ($index === 0) show @endif" data-bs-parent="#accordionExample">
                                                <div class="accordion-body">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" class="form-control" name="title[{{ $locale }}]"  value="{{ $plan_feature->getTranslation('title', $locale) }}" id="title_{{ $locale }}" placeholder="{{ __('Title in ') . strtoupper($locale) }}" required>
                                                    </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <select id="plan_id" required name="plan_id" class="form-control select2">
                                            <option value="">{{ __('Select Plan') }}</option>
                                            @foreach($plans as $plan)
                                                <option value="{{ $plan->id }}" {{$plan_feature->plan_id==$plan->id?'selected':''}}>{{ $plan->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <select id="status" required name="status" class="form-control select2">
                                            ...
                                            <option value="0" {{$plan_feature->status==0?'selected':''}}>{{__('Not Active')}}</option>
                                            <option value="1" {{$plan_feature->status==1?'selected':''}}>{{ __('Active') }}</option>
...
@section('scripts')
    ...
    <script>
        ...
        var data_url='{{ route('admin.plan_features.update',$plan_feature->id)}}'
